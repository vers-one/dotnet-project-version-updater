...
const updater_1 = __importDefault(__nccwpck_require__(3045));
async function run() {
    try {
        let filePathPattern = Core.getInput("file");
        if (!filePathPattern) {
...
class CsProjPlugin extends plugin_1.default {
    ...
    isFileTypeSupported(filePath) {
        return filePath.toLowerCase().endsWith(".csproj");
    }
}
...
var VersionPartDelimiter;
(function (VersionPartDelimiter) {
    VersionPartDelimiter[VersionPartDelimiter["DOT"] = 0] = "DOT";
            ...
            if (matches) {
                ...
                switch (tag.versionPartDelimiter) {
                    ...
                    case VersionPartDelimiter.COMMA:
                        ...
                        const newVersion = version_utils_1.default.updateParsedVersion(parsedOldVersion, versionUpdateRule);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const assemblyinfo_cpp_plugin_1 = __importDefault(__nccwpck_require__(5950));
const assemblyinfo_cs_plugin_1 = __importDefault(__nccwpck_require__(8336));
const assemblyinfo_fs_plugin_1 = __importDefault(__nccwpck_require__(738));
const assemblyinfo_vb_plugin_1 = __importDefault(__nccwpck_require__(5729));
const csproj_plugin_1 = __importDefault(__nccwpck_require__(6263));
const fsproj_plugin_1 = __importDefault(__nccwpck_require__(1586));
const nuspec_plugin_1 = __importDefault(__nccwpck_require__(1643));
const props_plugin_1 = __importDefault(__nccwpck_require__(1285));
const rc_plugin_1 = __importDefault(__nccwpck_require__(2140));
const vbproj_plugin_1 = __importDefault(__nccwpck_require__(3727));
const plugins = [
    new csproj_plugin_1.default(),
    new props_plugin_1.default(),
    new assemblyinfo_cs_plugin_1.default(),
    new nuspec_plugin_1.default(),
    new vbproj_plugin_1.default(),
    new assemblyinfo_vb_plugin_1.default(),
    new fsproj_plugin_1.default(),
    new assemblyinfo_fs_plugin_1.default(),
    new assemblyinfo_cpp_plugin_1.default(),
...
async function update(filePathPattern, newVersion, pluginTagFilterString) {
    ...
    const filePaths = await (0, fast_glob_1.default)(parsedFilePathPatterns, { dot: true });
...
function parseVersionUpdateRule(input) {
    ...
    let matches = VERSION_NUMBER_WITH_DOT_DELIMITER_REGEX.exec(input);
...
function updateNonParsedVersion(currentVersion, versionUpdateRule, versionNumberRegex, versionPartDelimiter) {
    if (versionUpdateRule.updateMode === version_types_1.VersionUpdateMode.SET_EXPLICIT_VERSION) {
        ...
        const currentParsedVersion = parseVersion(currentVersion, versionNumberRegex);
...
function updateParsedVersion(currentVersion, versionUpdateRule) {
    ...
    if (versionUpdateRule.updateMode === version_types_1.VersionUpdateMode.SET_EXPLICIT_VERSION) {
        ...
        const updatePattern = versionUpdateRule.updatePattern;
        const result = {
            major: (_a = getNewVersionPart(currentVersion.major, updatePattern.major)) !== null && _a !== void 0 ? _a : 0,
...
function parseVersionPartUpdateRule(input) {
    let result;
    if (input === "?") {
        result =
            {
        ...
        const partNumber = parseInt(input);
        if (!isNaN(partNumber)) {
            result =
                {
...
function parseVersion(version, versionNumberRegex) {
    const matches = versionNumberRegex.exec(version.trim());
    if (matches && matches[0] === version) {
        const major = parseInt(matches[1]);
        const minor = parseInt(matches[2]);
        const build = parseInt(matches[3]);
        const revision = parseInt(matches[4]);
        const result = {
            major,
...
function formatVersion(version, versionPartDelimiter = ".", includeTrailingZeros = false) {
    var _a, _b, _c;
    let result = version.major.toString();
    if (version.minor !== null || includeTrailingZeros) {
        result += versionPartDelimiter + ((_a = version.minor) !== null && _a !== void 0 ? _a : 0);
        if (version.build !== null || includeTrailingZeros) {
            result += versionPartDelimiter + ((_b = version.build) !== null && _b !== void 0 ? _b : 0);
            if (version.revision !== null || includeTrailingZeros) {
                result += versionPartDelimiter + ((_c = version.revision) !== null && _c !== void 0 ? _c : 0);
            }
        }
    }
    return result;
