...
async function run() {
    try {
        ...
        const updateResults = await updater_1.default.update(filePathPattern, newVersion);
...
const version_utils_1 = __importDefault(__nccwpck_require__(1534));
const VERSION_TAG_REGEX = /<Version>(.*)<\/Version>/i;
const ASSEMBLY_VERSION_ATTRIBUTE_REGEX = /\[assembly: AssemblyVersion\("(.*)"\)]/i;
const ASSEMBLY_FILE_VERSION_ATTRIBUTE_REGEX = /\[assembly: AssemblyFileVersion\("(.*)"\)]/i;
...
async function updateFile(filePath, versionUpdateRule) {
    ...
    for (const regex of regexes) {
        ...
        if (matches && matches.length == 2) {
            matchesFound = true;
            const oldVersionTag = matches[0];
            oldVersion = matches[1].trim();
            newVersion = version_utils_1.default.getNewVersion(oldVersion, versionUpdateRule);
            const newVersionTag = oldVersionTag.replace(oldVersion, newVersion);
...
function parseVersionUpdateRule(input) {
    ...
    let matches = VERSION_NUMBER_REGEX.exec(input);
...
function getNewVersion(currentVersion, versionUpdateRule) {
    var _a;
    ...
    version.major = (_a = getNewVersionPart(version.major, updatePattern.major)) !== null && _a !== void 0 ? _a : 0;
...
function parseVersion(version) {
    const matches = VERSION_NUMBER_REGEX.exec(version.trim());
